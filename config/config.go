	...
	if !GlobalConfig.init {
		...
		if !GlobalConfig.init {
			if PROD {
				...
				GlobalConfig = Config{PRIVATE_KEY, RPC_URL_HTTP, RPC_URL_WS, CHAIN_ID, WETH_ADDRESS, FLASH_QUERY_ADDRESS, BUNDLE_EXECUTOR_ADDRESS, UNISWAPV2_FACTORIES, UNISWAPV2_ROUTER02S, true}
			} else {
